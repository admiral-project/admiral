...
| Helpers rootless especializados y ciclo E2E | Single node (CentOS Stream 10, RPM `admiral-fleet-0.0.1beta19-9`) | Aprobado | Provision WordPress `op_85d8746964a9c5b3` / `inst_1bdfc1c8633a8592` correcto; backup MariaDB real en S3 `op_8f43a9e89de06c99` → `bk_7c69fe302c83c465` con checksum; pause `op_d57bce9a26b3173e`, restore `op_c99924894e0a9e7f` con verificación y start `op_23c7693e30475c28`, todos correctos. Inspección `op_6cb1de70ddb621be` confirmó `running` y `healthy`; HTTP de WordPress respondió 301. Lifecycle, setup, secrets, backup y restore usaron los helpers especializados como `admiral-apps`. |
| Issues de GitHub | `admiral-project/admiral` | En curso | La revisión upstream con `gh issue list` confirma cerrados #3, #4, #7, #8, #9, #11, #12, #13 y #15. El issue [#4](https://github.com/admiral-project/admiral/issues/4) quedó cerrado el 2026-08-01 con HTTPS real, validación TLS, `ADMIRAL_INSECURE_SKIP_VERIFY` desactivado, objeto remoto verificado y restore con checksum. Permanecen abiertos #5, #6, #10, #14, #16 y #17; no se mezclan con la validación S3 ya cerrada. |
...
### Revisión upstream del 1 de agosto de 2026

El gate S3/HTTPS dejó de ser un pendiente: upstream cerró el [issue #4](https://github.com/admiral-project/admiral/issues/4)
con evidencia de un endpoint S3-compatible real sobre HTTPS, verificación de
certificado/hostname, `ADMIRAL_INSECURE_SKIP_VERIFY=0`, existencia del objeto
remoto y restore con checksum. El issue #6 continúa abierto por el conjunto de
hallazgos de seguridad restante, pero su referencia histórica a M6/B1 no debe
interpretarse como que el requisito S3/TLS siga sin resolver.
